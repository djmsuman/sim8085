...
void reploop(CPU& cpu)
{
    ...
    while(true)
    {
        ...
        else if(cmd == "print" || cmd == "p")
        {
            ...
            else if(cmd == "lmem") printLoadedMemory(cpu);
            else if(cmd == "umem") printUsedMemory(cpu);
            else if(cmd == "r") printRegister(cpu);
            else if(cmd == "rf" || cmd == "fr")
            {
                printFlags(cpu);
                printRegister(cpu);
            }
            else if(cmd == "f" || cmd == "flags") printFlags(cpu);
            else if(cmd == "a") cout << hex << (uint)cpu.A << " binary: " << bitset<8>(cpu.A) << endl;
            else if(cmd == "b") cout << hex << (uint)cpu.B << " binary: " << bitset<8>(cpu.B) << endl;
            else if(cmd == "c") cout << hex << (uint)cpu.C << " binary: " << bitset<8>(cpu.C) << endl;
            else if(cmd == "d") cout << hex << (uint)cpu.D << " binary: " << bitset<8>(cpu.D) << endl;
            else if(cmd == "e") cout << hex << (uint)cpu.E << " binary: " << bitset<8>(cpu.E) << endl;
            else if(cmd == "h") cout << hex << (uint)cpu.H << " binary: " << bitset<8>(cpu.H) << endl;
            else if(cmd == "l") cout << hex << (uint)cpu.L << " binary: " << bitset<8>(cpu.L) << endl;
            else if(cmd == "bc") cout << hex << (uint)cpu.putBC() << " binary: " << bitset<16>((uint)cpu.putBC()) << endl;
            else if(cmd == "de") cout << hex << (uint)cpu.putDE() << " binary: " << bitset<16>((uint)cpu.putDE()) << endl;
            else if(cmd == "hl") cout << hex << (uint)cpu.putHL() << " binary: " << bitset<16>((uint)cpu.putHL()) << endl;
            else if(cmd == "pc") cout << hex << (uint)cpu.putPC() << " binary: " << bitset<16>((uint)cpu.putPC()) << endl;
            else if(cmd == "sp") cout << hex << (uint)cpu.putSP() << " binary: " << bitset<16>((uint)cpu.putSP()) << endl;
        }
            ...
            if(ss >> hex >> addrStart >> hex >> addrEnd)
            {
                if(expect_byte2(addrStart) && expect_byte2(addrEnd))
                {
                    ...
                    while(cpu.memory.currentInsPos() <= addrEnd)
                    {
                        ...
                        else if(cmd == "p") cpu.memory.jump(cpu.memory.currentInsPos() - 1);
                        else
                        {
        ...
        else if(cmd == "set" || cmd == "S")
        {
            ...
            if(cmd == "a" && expect_byte(arg)) cpu.A = static_cast<Byte>(arg);
            else if(cmd == "b" && expect_byte(arg)) cpu.B = static_cast<Byte>(arg);
            else if(cmd == "c" && expect_byte(arg)) cpu.C = static_cast<Byte>(arg);
            else if(cmd == "d" && expect_byte(arg)) cpu.D = static_cast<Byte>(arg);
            else if(cmd == "e" && expect_byte(arg)) cpu.E = static_cast<Byte>(arg);
            else if(cmd == "h" && expect_byte(arg)) cpu.H = static_cast<Byte>(arg);
            else if(cmd == "l" && expect_byte(arg)) cpu.L = static_cast<Byte>(arg);

            else if(cmd == "bc" && expect_byte2(arg)) cpu.BC = static_cast<Byte2>(arg);
            else if(cmd == "de" && expect_byte2(arg)) cpu.DE = static_cast<Byte2>(arg);
            else if(cmd == "hl" && expect_byte2(arg)) cpu.HL = static_cast<Byte2>(arg);
            else if(cmd == "pc" && expect_byte2(arg)) cpu.PC = static_cast<Byte2>(arg);
            else if(cmd == "sp" && expect_byte2(arg)) cpu.SP = static_cast<Byte2>(arg);

            else if(cmd == "s") cpu.S = arg & 0x1;
            else if(cmd == "z") cpu.Z = arg & 0x1;
            else if(cmd == "ac") cpu.AC = arg & 0x1;
            else if(cmd == "p") cpu.P = arg & 0x1;
            else if(cmd == "cy") cpu.CY = arg & 0x1;
            ...
            else if(cmd == "mem" && expect_byte2(arg))
            {
                ...
                if(!(ss >> hex >> val))
                {
                    ...
                    //cout << hex << val  << endl;
                    if(!expect_byte(val)) cout << "value too big. Expected a byte(max FF)" << endl;
                    else cpu.memory[arg] = val;
            }
            else
